...
4. Verify each component against its baseline via image diff on the matching surface, driving the real app with the bundled `/run` and `/verify` skills. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. The bundled `/loop` skill per component until the diff is zero.
