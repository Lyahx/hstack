...
2. Pick the wake mechanism using the bundled `/loop` skill (Claude Code ships it; it is not a pstack skill). `/loop <interval> <prompt>` fires on a fixed interval, and `/loop` with no interval lets the model self-pace between iterations. An event to watch (CI, a merge, a ref advancing) gets a watcher subagent that wakes you on the event, with a long time-based heartbeat as fallback. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking.
