...
6. **Verify the chain from transcripts, not self-report.** Read each candidate's local transcript under this project's transcript directory, `~/.claude/projects/<project>/<session>.jsonl` plus `<session>/subagents/*.jsonl`. Resolve `<project>` for the current project only, per `${CLAUDE_PLUGIN_ROOT}/skills/reflect/SKILL.md` step 1. Do not glob across `~/.claude/projects/*/`; that crosses project boundaries and reads private transcripts from unrelated work. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
